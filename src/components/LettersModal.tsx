...
import { 
  ...
  ArrowLeft, Paperclip, FileText, Image as ImageIcon, Video, Music, Plus
} from 'lucide-react';
...
interface Letter {
  ...
  content: string | null;
  attachments?: string[];
  // Для поддержки старых чат-сообщений:
...
export function LettersModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  // Режимы: 'inbox' (Ящик), 'compose' (Написать), 'read' (Читать)
  ...
  // Состояния для написания письма
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime_letters')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'letters', filter: `space_id=eq.${spaceId}` }, (payload) => {
        ...
        if (newL.sender_id !== currentUserId) setLetters(prev => [newL, ...prev]);
      })
  ...
  }, [spaceId, isOpen, currentUserId, selectedLetter]);
  ...
  const handleSendLetter = async () => {
    ...
    try {
      ...
      // Загружаем голосовое
      if (composeVoice) {
        uploadedUrls.push(await uploadFile(composeVoice, 'webm'));
      }
      ...
      const { data, error } = await supabase.from('letters').insert({
        ...
        is_read: false,
        type: 'text' // оставляем для совместимости со старым кодом
      }).select('*').single();
      ...
      // Сброс формы и возврат во входящие
      setComposeTitle('');
      setComposeFiles([]);
      ...
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-gray-900/40 backdrop-blur-sm flex items-center justify-center p-0 sm:p-6">
          <motion.div initial={{ y: 20, opacity: 0, scale: 0.95 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 20, opacity: 0, scale: 0.95 }} transition={{ type: "spring", duration: 0.5, bounce: 0 }} className="bg-gray-50 w-full max-w-4xl h-full sm:h-[90vh] flex flex-col shadow-2xl relative sm:rounded-3xl overflow-hidden">
            ...
            {/* ── Шапка ── */}
            <div className="flex items-center justify-between p-5 sm:px-8 border-b border-gray-200 bg-white sticky top-0 z-10 shrink-0">
              <div className="flex items-center gap-4">
                {view !== 'inbox' ? (
                  <button onClick={() => setView('inbox')} className="p-2 -ml-2 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-colors">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-500">
                    {view === 'inbox' && `${letters.length} посланий`}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                {view === 'inbox' && (
                  <button onClick={() => setView('compose')} className="hidden sm:flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-xl hover:bg-black transition-colors">
                    Написать
                  </button>
                )}
                <button onClick={onClose} className="p-2 bg-white border border-gray-200 rounded-full hover:bg-gray-100 text-gray-400 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="flex-grow overflow-y-auto bg-gray-50 relative">
              ...
              {view === 'inbox' && (
                <div className="p-4 sm:p-8">
                  {isLoading && letters.length === 0 ? (
                    ...
                    <div className="text-center py-20 text-gray-400">В ящике пока пусто. Напишите первое письмо!</div>
                  ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between text-xs font-medium text-gray-400">
                              <span>{new Date(letter.created_at).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long' })}</span>
                  ...
                  {hasMore && (
                    <button onClick={() => { setPage(p => p + 1); fetchLetters(page + 1); }} className="mx-auto mt-8 block px-5 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50">
                      Загрузить еще
                    </button>
                  )}
                </div>
              ...
              {view === 'read' && selectedLetter && (
                <div className="max-w-2xl mx-auto p-6 sm:p-12 bg-white min-h-full shadow-sm">
                  <div className="text-center mb-10 pb-10 border-b border-gray-100">
                    ...
                    <p className="text-sm font-medium text-terra-600 mt-4">
                      От: {selectedLetter.sender_id === currentUserId ? 'Вас' : (profile?.display_name || 'Партнера')}
                    </p>
                  </div>
                  ...
                  {/* Вложения старого формата (Legacy) */}
                ...
                <div className="max-w-3xl mx-auto p-4 sm:p-8">
                  <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-gray-100">
                    ...
                    <input 
                      ...
                      className="w-full text-3xl font-semibold text-gray-900 placeholder:text-gray-300 outline-none mb-8 bg-transparent"
                    />
                      ...
                      <div className="mt-8 flex flex-wrap gap-3">
                        {composeFiles.map((file, i) => (
                          <div key={i} className="flex items-center gap-2 bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg text-sm text-gray-600">
                            ...
                            <button onClick={() => setComposeFiles(prev => prev.filter((_, idx) => idx !== i))} className="ml-2 hover:text-red-500"><X className="w-4 h-4"/></button>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="mt-10 pt-6 border-t border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                        ...
                        <button onClick={() => fileInputRef.current?.click()} className="p-3 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-900 transition-colors tooltip flex-shrink-0" title="Прикрепить файл">
                      ...
                      <button 
                        ...
                        disabled={isSending}
                        className="w-full sm:w-auto px-8 py-3.5 bg-gray-900 text-white rounded-xl hover:bg-black font-medium transition-all shadow-md disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 flex-shrink-0"
                      >
            ...
            {/* Мобильная кнопка написать (плавающая) */}
            {view === 'inbox' && (
              <button 
                onClick={() => setView('compose')} 
                className="sm:hidden absolute bottom-6 right-6 p-4 bg-gray-900 text-white rounded-full shadow-xl hover:bg-black z-20"
              >
